...
export default function AdminHomePage() {
  ...
  const handleBlockUser = async (userId: string, blocked: boolean, days: number = 0) => {
    try {
      const res = await fetch("/api/admin/block-user", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ userId, blocked, days }),
      });
         ...
         {loading ? (
           ...
           >
             {filteredUsers.map((user, index) => (
               ...
               >
                 <div onClick={() => router.push(`/${user._id}`)} className="flex items-center space-x-4">
                   <img 
